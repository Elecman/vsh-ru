...
#define PLUGIN_VERSION       "VSH 2.0"
#define PLUGIN_DESCRIPT      "URES | VSH IMPERIA"
...
enum struct VSH2ModuleSys {
	...
	bool IsPluginABoss(Handle plugin) {
		...
		for( int i; i<len; i++ ) {
			BossModule module;
			this.m_hBossesRegistered.GetArray(i, module, sizeof(module));
			if( module.plugin==plugin )
				return true;
		}
		return false;
	}
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_vmusic", Command_ToggleMusic, "Toggles your vsh music.");
	...
	g_vsh2.m_hCvars.AliveToEnable = CreateConVar("vsh2_point_alive", "5", "Enable control points when there are X people left alive.", FCVAR_NOTIFY, false, 0.0, false, 999999.0);
	...
	g_vsh2.m_hCvars.AirblastRage = CreateConVar("vsh2_airblast_rage", "8.0", "How much Rage should airblast give/remove? (negative number to remove rage)", FCVAR_NOTIFY, true, 0.0, true, 100.0);
	g_vsh2.m_hCvars.JarateRage = CreateConVar("vsh2_jarate_rage", "8.0", "How much rage should Jarate give/remove? (negative number to add rage)", FCVAR_NOTIFY, true, 0.0, true, 100.0);
	g_vsh2.m_hCvars.FanoWarRage = CreateConVar("vsh2_fanowar_rage", "5.0", "How much rage should the Fan o' War give/remove? (negative number to add rage)", FCVAR_NOTIFY, true, 0.0, true, 100.0);
	...
	g_vsh2.m_hCvars.MedievalRespawnTime = CreateConVar("vsh2_medievalmode_respawntime", "5.0", "How long it takes for players to respawn after dying in medieval mode (if they have live left).", FCVAR_NOTIFY, true, 1.0, true, 999.0);
	g_vsh2.m_hCvars.PermOverheal = CreateConVar("vsh2_permanent_overheal", "0", "If enabled, Mediguns give permanent overheal.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_vsh2.m_hCvars.MultiCapture = CreateConVar("vsh2_multiple_cp_captures", "1", "If enabled, allow control points to be captured more than once instead of ending the round instantly.", FCVAR_NOTIFY, false, 0.0, false, 1.0);
	g_vsh2.m_hCvars.MultiCapAmount = CreateConVar("vsh2_multiple_cp_capture_amount", "3", "If vsh2_allow_multiple_cp_captures is enabled, how many times must a team capture a Control Point to win.", FCVAR_NOTIFY, false, 0.0, false, 999999.0);
	...
	g_vsh2.m_hCvars.BlockEureka = CreateConVar("vsh2_allow_eureka_effect", "0", "Enables/Disables the Eureka Effect for Engineers", FCVAR_NONE, true, 0.0, true, 1.0);
	g_vsh2.m_hCvars.ForceLives = CreateConVar("vsh2_force_player_lives", "0", "Forces the g_vsh2.m_hGamemode to apply Medieval Mode lives on players, whether or not medieval mode is enabled", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	g_vsh2.m_hCvars.SydneySleeperRageRemove = CreateConVar("vsh2_sydney_sleeper_rage_remove", "0.01", "how much rage (multiplied with damage) the Sydney Sleeper sniper rifle will remove from a boss' rage meter.", FCVAR_NONE, true, 0.0, true, 99.0);
	...
	g_vsh2.m_hCvars.DeadRingerDamage = CreateConVar("vsh2_dead_ringer_damage", "90.0", "damage, divided by 0.25, that dead ringer spies will take from boss melee hits.", FCVAR_NONE, true, 0.0, false);
	g_vsh2.m_hCvars.CloakDamage = CreateConVar("vsh2_cloak_damage", "70.0", "damage, divided by 0.8, that dead ringer spies will take from boss melee hits.", FCVAR_NONE, true, 0.0, false);
	...
	g_vsh2.m_hCvars.CapReenableTime = CreateConVar("vsh2_multiple_cp_capture_reenable_time", "30.0", "time to reenable the control pointer after being captured, does nothing is 'vsh2_multiple_cp_captures' is disabled.", FCVAR_NONE, false, 0.0, false);
	...
	musicCookie = RegClientCookie("ff2 music", "Toggles your vsh music.", CookieAccess_Public);
...
public Action CheckLateSpawn(int client, const char[] command, int argc)
{
	...
	if( !g_vsh2.m_hCvars.AllowLateSpawn.BoolValue && GetClientTeam(client) > VSH2Team_Spectator && TF2_GetPlayerClass(client)==TFClass_Unknown ) {
		char str_tfclass[20]; GetCmdArg(1, str_tfclass, sizeof(str_tfclass));
		TFClassType classtype = TF2_GetClass(str_tfclass);
		CPrintToChat(client, "{olive}[VSH 2]{default} Поздний спавн заблокирован.");
	...
	if( g_vsh2.m_hCvars.Enabled.BoolValue && g_vsh2.m_hGamemode.iRoundState == StateRunning ) {
		...
		if( player.bIsBoss ) {
			...
			if( flhp_percent > g_vsh2.m_hCvars.SuicidePercent.FloatValue ) {
				CPrintToChat(client, "{olive}[VSH 2]{default} Вы не можете сейчас умереть. Напишите '!resetq' instead.");
				return Plugin_Handled;
			}
		} else {
...
public void OnConfigsExecuted()
{
	...
	if( VSHGameMode.IsVSHMap() ) {
		g_oldcvar_vals.tf_arena_use_queue = FindConVar("tf_arena_use_queue").IntValue;
		g_oldcvar_vals.mp_teams_unbalance_limit = FindConVar("mp_teams_unbalance_limit").IntValue;
		g_oldcvar_vals.tf_arena_first_blood = FindConVar("tf_arena_first_blood").IntValue;
		g_oldcvar_vals.mp_forcecamera = FindConVar("mp_forcecamera").IntValue;
		g_oldcvar_vals.tf_scout_hype_pep_max = FindConVar("tf_scout_hype_pep_max").FloatValue;
		...
		FindConVar("mp_teams_unbalance_limit").IntValue =  g_vsh2.m_hCvars.FirstRound.BoolValue ? 0 : 1;
		FindConVar("mp_forceautoteam").IntValue = g_vsh2.m_hCvars.FirstRound.BoolValue ? 0 : 1;
...
public void ConnectionMessage(const int userid)
{
	...
	if( IsValidClient(client) ) {
		CPrintToChat(client, "{olive}[VSH 2]{default} Добро пожаловать на VSH2, напишите /bosshelp для помощи!");
...
public void OnMapStart()
{
	...
	//g_vsh2.m_hGamemode.hMusic = null;
...
public Action Timer_PlayerThink(Handle hTimer)
{
	...
	for( int i=MaxClients; i; --i ) {
		...
		if (!IsPlayerAlive(i)) SetHudTextParams(-1.0, 0.22, 0.11, 255, 255, 255, 255);
		else SetHudTextParams(-1.0, 0.12, 0.11, 255, 255, 255, 255);
		
		
		ShowSyncHudText(i, g_vsh2.m_hHUDs[HealthHUD], "%i / %i", VSHGameMode.GetTotalBossHealth(), g_vsh2.m_hGamemode.iTotalMaxHealth); //---------------------------------
		//}
	}
...
public void ShowPlayerScores()
{
	...
	for( int i=MaxClients; i; --i ) {
		...
		if( !(GetClientButtons(i) & IN_SCORE) ) {
			player = BaseBoss(i);
			ShowHudText(i, -1, "Больше всего нанесли урон:\n1)%i - %s\n2)%i - %s\n3)%i - %s\n\nНанесено Урона: %i", hTop[0].iDamage, score1, hTop[1].iDamage, score2, hTop[2].iDamage, score3, player.iDamage);
		}
	}
}
...
public void CalcScores()
{
	...
	for( int i=MaxClients; i; --i ) {
		...
		if( player.bIsBoss )
			...
			player.iQueue += queue;
			CPrintToChat(i, "{olive}[VSH 2] Queue{default} Вы получили %i очков очереди.", queue);
			CPrintToChat(i, "{olive}[VSH 2] Queue{default} Вы получили %i очков к общему счёту.", points);
		}
	}
}
...
{
	if( g_vsh2.m_hGamemode.iHealthBar.iPercent < g_vsh2.m_hCvars.HealthPercentForLastGuy.IntValue || g_vsh2.m_hGamemode.iRoundState != StateRunning || g_vsh2.m_hGamemode.iTimeLeft < 0 )
		return Plugin_Stop;
	...
	else g_vsh2.m_hGamemode.iTimeLeft--;
...
public void _ResetMediCharge(const int entid)
{
	...
	if( medigun > MaxClients && IsValidEntity(medigun) )
		SetMediCharge(medigun, GetMediCharge(medigun) + g_vsh2.m_hCvars.MedigunReset.FloatValue);
}
...
public void _MusicPlay()
{
	...
	if( g_vsh2.m_hGamemode.flMusicTime > currtime )
		return;
...
public int RegisterBoss(Handle plugin, const char modulename[MAX_BOSS_NAME_SIZE])
{
	...
	module.plugin = plugin;
	g_modsys.m_hBossesRegistered.PushArray(module, sizeof(module));
	return MAXBOSS;
}
...
public any Native_GetBossIDs(Handle plugin, int numParams)
{
	...
	if( !registered_only ) {
		...
		boss_map.SetValue("yeti", VSH2Boss_Yeti);
	}
...
{
	bool alive = GetNativeCell(1);
	return VSHGameMode.CountMinions(alive);
}
...
public int Native_VSH2GameMode_GetMinions(Handle plugin, int numParams)
{
	...
	bool balive = GetNativeCell(2);
	int numminions = VSHGameMode.GetMinions(minions, balive);
