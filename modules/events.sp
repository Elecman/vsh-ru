...
public Action RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	g_vsh2.m_hGamemode.GetBossType();    /// in g_vsh2.m_hGamemode.sp
	...
	if( GetClientCount() <= 1 || playing < 2 ) {
		CPrintToChatAll("{olive}[VSH 2]{default} Нужно больше Игроков чтобы начать");
		...
		CPrintToChatAll("{olive}[VSH 2]{default} Обычный раунд (пока все загружаются)");
		g_vsh2.m_hGamemode.iRoundState = StateDisabled;
		SetArenaCapEnableTime(60.0);
...
public Action ArenaRoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	for( i=MaxClients; i; --i ) {
		...
		int max_health = CalcBossHealth(1046.8, red_players, 1.0, 1.0341, 3800.0) / (bosscount);    /// In stocks.sp
