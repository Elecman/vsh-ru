...
#define HALESPEED		350.0

#define HALE_JUMPCHARGE		(25*1.0)
#define HALERAGEDIST		800.0
#define HALE_WEIGHDOWN_TIME	3.5

#define HaleTheme "imperia_music/saxton_bgm294.mp3"
#define HaleTheme2 "imperia_music/saxton_bgm2941.mp3"
...
methodmap CHale < BaseBoss {
	...
	{
		strcopy(snd, PLATFORM_MAX_PATH, HaleTheme);
		EmitSoundToClient(client, snd);
	}
	
	public void PlayBgm2(int client)
	{
		strcopy(snd, PLATFORM_MAX_PATH, HaleTheme2);
		EmitSoundToClient(client, snd);
	}
	...
	public void Equip() {
		this.SetName("Сакстон Хейл");
		this.RemoveAllItems();
		char attribs[128];
		Format(attribs, sizeof(attribs), "2; 3.1; 259; 1.0; 252 ; 0.50; 32 ; 0.60; 214; %d", GetRandomInt(999, 9999));
		int SaxtonWeapon = this.SpawnWeapon("tf_weapon_shovel", 195, 100, 5, attribs);
...
public void AddHaleToDownloads()
{
	...
	PrecacheSound(HaleTheme);
	
	PrepareSound(HaleTheme2);
	PrecacheSound(HaleTheme2);
...
public void AddHaleToMenu(Menu& menu)
{
	...
	menu.AddItem(bossid, "Сакстон Хейл.");
}
