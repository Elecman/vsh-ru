#define HHHModel			"models/player/new_saxton_hale/kingofskulls/kingofskullsv3.mdl"
...
#define HHHSPEED		358.0
#define HALEHHH_TELEPORTCHARGETIME     2
#define HALEHHH_TELEPORTCHARGE         (25.0 * HALEHHH_TELEPORTCHARGETIME)
#define HHHRAGEDIST	1000.0

#define HHHTheme "imperia_music/thisIshhalloween.mp3"
...
methodmap CHHHJr < BaseBoss {
	...
	{
		strcopy(snd, PLATFORM_MAX_PATH, HHHTheme);
	...
	public void Think()
	{
		...
		if( ((buttons & IN_DUCK) || (buttons & IN_ATTACK2)) && (this.flCharge >= 0.0) ) {
			...
			if( (this.flCharge == HALEHHH_TELEPORTCHARGE || this.bSuperCharge) && EyeAngles[0] < -5.0 ) {
				...
				if( target != -1 ) {
					...
					PrintCenterText(target, "К вам телепортировались");
		...
		if( (buttons & IN_DUCK) && this.flWeighDown >= 2.0) {
		...
		if( this.flRAGE >= 100.0 )
			ShowSyncHudText(this.index, g_vsh2.m_hHUDs[PlayerHUD], "Телепорт: %i%% | Удары: %i / %i | Ярость: НАКОПЛЕНА - Позовите Медика (клавиша: E) для активации", this.bSuperCharge ? 1000 : RoundFloat(jmp) * 2, this.iClimbs, max_climbs);
		else ShowSyncHudText(this.index, g_vsh2.m_hHUDs[PlayerHUD], "Телепорт: %i%% | Удары: %i / %i | Ярость: %0.1f", this.bSuperCharge ? 1000 : RoundFloat(jmp) * 2, this.iClimbs, max_climbs, this.flRAGE);
	}
	...
	public void Equip() {
		this.SetName("Король мёртвых");
		this.RemoveAllItems();
		char attribs[128];
		
		Format(attribs, sizeof(attribs), "2 ; 3.1; 259 ; 1.0; 1005 ; 3100495; 252 ; 0.50; 781 ; 1.0; 551 ; 1; 6 ; 0.90");
		int SaxtonWeapon = this.SpawnWeapon("tf_weapon_sword", 1082, 100, 5, attribs);
	...
	public void Help()
	{
		if( IsVoteInProgress() )
			return;
		char helpstr[] = "Пeший Bcaдник бeз Гoлoвы.:\nTeлeпopт: cмoтpитe ввepx c зaжaтым ПКМ (зaтeм oтпycтитe).\nЯpocть (Cтaн): oглушaeт игpoкoв вoкpуг вac.\nПpыжки: бeйтe пo любoмy oбъeктy чтoб oтпpыгнyть oт нeгo";
...
public void AddHHHToMenu(Menu& menu)
{
	...
	menu.AddItem(bossid, "Король мёртвых");
}
